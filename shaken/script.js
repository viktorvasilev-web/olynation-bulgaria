...
testingRegistrationForm.addEventListener('submit', event => {
  ...
  submitToSheet(testingRegistrationForm, `Shaken Massager | Безплатно тестване | Дата: ${eventDate} | Поканен от: ${inviter} | Страница: ${window.location.href}`);
...
orderForm.addEventListener('submit', event => {
  ...
  const details = [
    'ПОРЪЧКА',
    'Продукт: OlyLife Shaken Massager — USD 1,000*',
  ...
  closeOrder();
  showThankYou('Заявката ти за OlyLife Shaken Massager беше изпратена успешно. Ще се свържа с теб за потвърждение и следващите стъпки.');
});
